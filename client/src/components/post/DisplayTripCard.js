...
function DisplayPostCard({ post, parentAction, deletePost }) {
        ...
        <Link to={postLink}>
          <div className="d-flex justify-content-between">
            ...
            {parentAction === "searchTrips" ? (
              ...
              (<span className="post-time">{moment(createdAt).fromNow(true)} {AGO}</span>)
            }
          </div>
        </Link>
  ...
  return (
    <>
      <Header className="post-pointer" image>
        <Header.Content>
          <Link to={postLink}>
            <>
              <Responsive maxWidth={991}>
                <Header as="h2" className="fixline">{title}</Header>
              </Responsive>
              <Responsive minWidth={992}>
                <Header className="fixline" as="h2">{title}</Header>
              </Responsive>
            </>
          </Link>
