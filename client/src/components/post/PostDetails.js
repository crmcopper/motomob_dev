    ...
    <>
      {
        ...
        <div className="trip-head">
          <button className="ui button prev-btn" disabled={!hasPrev} onClick={e => { nextPrevAction("prev") }}><Icon name='chevron left' /> Prev</button>
          <div className="trip-name"><a href={"/trips/" + userId + "/" + tripName}>{tripName}</a></div>
          <button className="ui button next-btn" disabled={!hasNext} onClick={e => { nextPrevAction("next") }}>Next <Icon name='chevron right' /></button>
        </div>
      }
      {
        post && tripName &&
        <div className="text-center text-light trip-count">{current}/{total}</div>
      }
